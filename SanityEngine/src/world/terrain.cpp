...
namespace sanity::engine {
    ...
    RX_CONSOLE_IVAR(cvar_max_terrain_tile_distance,
                    "t.MaxTileDistance",
                    "Maximum distance at which Sanity Engine will load terrain tiles",
                    1,
                    INT_MAX,
                    16);
        ...
        for(Uint32 y = 0; y < tile_heightmap.size(); y++) {
            ...
            for(Uint32 x = 0; x < tile_heightmap_row.size(); x++) {
                ...
                if(x < tile_heightmap_row.size() - 1 && y < tile_heightmap.size() - 1) {
                    ...
                    // TODO: Triangulate the terrain mesh such that the vertices joined by an edge have more similar normals the the
    ...
    void Terrain::upload_new_tile_meshes() {
        ...
        commands->SetName(L"Terrain::upload_new_tile_meshes");
        {
            ...
            locked_tile_mesh_queue->each_fwd([&](const TerrainTileMeshCreateInfo& create_info) {
                ...
                const auto
                    cull_info = renderer::VisibleObjectCullingInformation{.aabb_x_min_max = {static_cast<float>(create_info.tilecoord.x),
        ...
        {
            TracyD3D12Zone(renderer::RenderBackend::tracy_context,
                           commands.Get(),
                           "Terrain::upload_new_tile_meshes::upload_visible_objects");
