...
namespace sanity::engine {
    ...
    void World::generate_climate_data(TerrainData& terrain_data, const WorldParameters& params, renderer::Renderer& renderer) {
         ...
         *
         * Wind is based on atmospheric circulation. The wind currents in Sanity Engine are very similar to the wind currents on Earth.
         * After computing the atmospheric circulation, the compute pass looks for features in the heightmap that would block the wind -
         * mountains, mostly. We search along the wind direction, looking for the horizon of the terrain. The closer and higher that horizon
         * is, the more wind it'll block. Wind gets output into a RGBA32F texture. The RGB holds the wind direction and strength - length of
         * the vector is the wind's speed in kmph
         *
         * Once we've determined wind speed and direction, we calculate humidity. There's a lot of humidity directly over water, then we
         * give it is 10km (or so) blur, then smear humidity along wind direction. Mountains will be able to block humidity in some way -
         * probably something about decreasing humidity amount by height and slope of the terrain is passes over? Humidity gets stored in a
         * R8 texture
         *
         ...
         * part of the temperature range, while humidity that isn't from a body of water will both raise and lower max temperature.
         * Temperature range gets saved to a RG8 texture - min in the red, max in the green
         *
