...
namespace sanity::engine::renderer {
    ...
    private:
        ...
        void set_buffer_indices(ID3D12GraphicsCommandList4* commands, Uint32 frame_idx) const;
            	
        void apply_advection(ID3D12GraphicsCommandList4* commands);

        void apply_buoyancy(ID3D12GraphicsCommandList4* commands);

        void apply_impulse(ID3D12GraphicsCommandList4* commands);
    };
} // namespace sanity::engine::renderer
