...
namespace Rx {
    ...
    struct Profiler {
        ...
        struct Device {
            ...
            constexpr Device(void* _context,
                             SetThreadNameFn _set_thread_name_fn,
                             BeginSampleFn _begin_sample_fn,
    ...
    inline constexpr Profiler::Sample::Sample(const SourceLocation& _source_location, const char* _tag)
        : m_source_location{_source_location}, m_tag{_tag}, m_enframing_destruct{nullptr}, m_nat{} {}
    ...
    inline const SourceLocation& Profiler::Sample::source_location() const& { return m_source_location; }

    inline const char* Profiler::Sample::tag() const { return m_tag; }
    ...
    // CPUSample
    inline Profiler::CPUSample::CPUSample(const SourceLocation& _source_location, const char* _tag) : Sample{_source_location, _tag} {
        instance().begin_cpu_sample(this);
    }

    inline Profiler::CPUSample::~CPUSample() { instance().end_cpu_sample(this); }

    // GPUSample
    inline Profiler::GPUSample::GPUSample(const SourceLocation& _source_location, const char* _tag) : Sample{_source_location, _tag} {
        instance().begin_gpu_sample(this);
    }

    inline Profiler::GPUSample::~GPUSample() { instance().end_gpu_sample(this); }
    ...
    inline void Profiler::bind_gpu(const GPU& _gpu) { m_gpu = _gpu; }

    inline void Profiler::bind_cpu(const CPU& _cpu) { m_cpu = _cpu; }

    inline void Profiler::unbind_gpu() { m_gpu = nullopt; }

    inline void Profiler::unbind_cpu() { m_cpu = nullopt; }

    inline constexpr Profiler& Profiler::instance() { return *s_instance; }
...
#define RX_PP_PASTE(_a, _b) _a##_b

#define RX_PP_CAT(_a, _b) RX_PP_PASTE(_a, _b)

#define RX_PP_UNIQUE(_base) RX_PP_CAT(_base, __LINE__)
