...
namespace Rx::Filesystem {
    ...
    Directory::Directory(Memory::Allocator& _allocator, String&& path_) : m_allocator{&_allocator}, m_path{Utility::move(path_)} {
#if defined(RX_PLATFORM_POSIX)
        ...
        Vector<Uint16> path_data{allocator(), path_utf16.size() + sizeof k_path_extra, Utility::UninitializedTag{}};
    ...
    void Directory::each(Function<void(Item&&)>&& _function) {
        ...
        for(;;) {
            // Skip '.' and '..'.
            if(context->find_data.cFileName[0] == L'.' && !context->find_data.cFileName[1 + !!(context->find_data.cFileName[1] == L'.')]) {
            ...
            const Item::Type kind = context->find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ? Item::Type::k_directory :
                                                                                                     Item::Type::k_file;
