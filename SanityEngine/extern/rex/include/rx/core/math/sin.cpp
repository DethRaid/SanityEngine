...
static constexpr const Float64 k_s1{-0x15555554cbac77.0p-55}; // -0.166666666416265235595
static constexpr const Float64 k_s2{0x111110896efbb2.0p-59};  //  0.0083333293858894631756
static constexpr const Float64 k_s3{-0x1a00f9e2cae774.0p-65}; // -0.000198393348360966317347
static constexpr const Float64 k_s4{0x16cd878c3b46a7.0p-71};  //  0.0000027183114939898219064

Float32 cosdf(Float64 _x); // cos.cpp
Float32 sindf(Float64 _x) {
  const Float64Eval z{_x * _x};
  const Float64Eval w{z * z};
  const Float64Eval r{k_s3 + z * k_s4};
  const Float64Eval s{z * _x};
  return static_cast<Float32>((_x + s * (k_s1 + z * k_s2)) + s * w * r);
}
...
static constexpr const Float64 k_pio2_1t{1.58932547735281966916e-08};
...
Sint32 rempio2(Float32 _x, Float64& y_) {
  ...
  // |_x| ~< 2^28*(pi/2)
  if (ix < 0x4dc90fdb) {
    const auto fn{static_cast<Float64Eval>(_x) * k_invpio2 + k_toint - k_toint};
