...
void main(const int3 id : SV_DispatchThreadID) {
	...
	const Texture3D divergence_in = textures3d[fluid_volume.temp_data_buffer];
	const float divergence = divergence_in[id].x;

	const RWTexture3D<float4> pressure_out = uav_textures3d_rgba[fluid_volume.pressure_textures[1]];
	
    pressure_out[id].x = (L + R + B + T + U + D - divergence) / 6.0;
}
