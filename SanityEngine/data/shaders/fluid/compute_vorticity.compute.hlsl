...
void main(const int3 id : SV_DispatchThreadID) {
    ...
    const float3 vorticity = 0.5 * float3(((T.z - B.z) - (U.y - D.y)), ((U.x - D.x) - (R.z - L.z)), ((R.y - L.y) - (T.x - B.x)));

	const RWTexture3D<float4> vorticity_out = uav_textures3d_rgba[fluid_volume.temp_data_buffer];
	
    vorticity_out[id].xyz = vorticity;
}
